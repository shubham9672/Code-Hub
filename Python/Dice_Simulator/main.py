...
def draw_window(dice, index):
    Dice_Image = pygame.image.load(
        os.path.join("images", "d{}.png".format(index))
    )  # including image using os
    ...
    win.blit(
        pygame.transform.scale(Dice_Image, (200, 200)), (dice.x, dice.y)
    )  # Adding the image of the dice
...
def main():
    dice = pygame.Rect(
        160, 150, 200, 200
    )  # creating a rectangle in which we have to place the image
    ...
    while run:
        ...
        for (
            ...
            pygame.event.get()
        ):  # reading event from the screen to keep track if ther any command is given to roll the dice
            ...
            ):  # To keep track of any mouse button pressed
                mouse_pos = event.pos
                if dice.collidepoint(
                    mouse_pos
                ):  # It became true when dice rectangle coordinates are same of the mouse pointer
